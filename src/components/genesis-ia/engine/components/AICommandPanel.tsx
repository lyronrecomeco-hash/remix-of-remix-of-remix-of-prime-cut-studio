...
import {
  ...
  Search, LineChart, MessageCircleMore
} from 'lucide-react';
...
export const AICommandPanel = ({
  ...
  nodes = [], lastActionType, onAutoArrange, onOpenWhatsApp,
}: AICommandPanelProps) => {
  ...
  const quickActions = useMemo(() => {
    if (!canvasInfo.hasStructure) {
      return QUICK_ACTIONS.filter((action) => ['build_structure', 'enrich_context', 'analyze'].includes(action.type));
    }
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2.5 border-b border-white/[0.06] flex-shrink-0 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2 text-[10px] text-white/25">
          <span>{outputs.length} documento(s) gerado(s)</span>
          <div className="flex items-center gap-2">
        ...
        <div ref={scrollRef} className="p-3 space-y-3">
          ...
          {messages.length === 0 ? (
            <div className="rounded-xl border border-dashed border-white/[0.08] bg-white/[0.02] p-5 text-center">
              ...
              <p className="text-sm font-medium text-white/75">Converse com a IA do Engine</p>
              <p className="mt-1 text-xs leading-relaxed text-white/30">
                Peça estratégia, análise, refinamento do canvas, copy de WhatsApp ou o build spec completo.
              </p>
            </div>
            ...
            <div key={message.id} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[92%] rounded-2xl border px-3 py-2.5 ${message.role === 'user' ? 'border-primary/25 bg-primary/12 text-white/90' : 'border-white/[0.06] bg-white/[0.03] text-white/75'}`}>
                ...
                {message.role === 'user' ? (
                  ...
                  <div className="prose prose-invert prose-xs max-w-none break-words leading-relaxed [&_h1]:text-sm [&_h1]:font-semibold [&_h2]:mt-4 [&_h2]:text-sm [&_h2]:font-semibold [&_h3]:mt-3 [&_h3]:text-xs [&_li]:my-0.5 [&_p]:my-2 whitespace-pre-wrap">
                    <ReactMarkdown>{message.content || (message.isStreaming ? 'Gerando resposta...' : '')}</ReactMarkdown>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className="border-t border-white/[0.06] bg-white/[0.02] p-3 flex-shrink-0">
        <div className="rounded-2xl border border-white/[0.08] bg-white/[0.03] p-2.5">
          <textarea
            ...
            rows={3}
            placeholder="Peça qualquer coisa para a IA: estratégia, copy, estrutura, refinamento do canvas ou build spec..."
            className="min-h-[84px] w-full resize-none bg-transparent px-1 text-xs text-white/80 placeholder:text-white/22 focus:outline-none"
          />
