  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
              ...
              >
                <div className="flex flex-col items-center gap-2 sm:gap-3">
                  ...
                  <div className="text-center">
                    <p className="text-xs sm:text-sm font-medium">{preset.name}</p>
        ...
        {/* Custom Colors */}
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-4 p-3 rounded-lg bg-white/5 border border-white/10">
          ...
          {themeModes.map(({ id, icon: Icon, label }) => {
            ...
            return (
              <button
                ...
                className={`relative p-3 sm:p-4 rounded-xl border flex flex-col items-center gap-1 sm:gap-2 transition-all ${
                  isSelected
                    ? 'bg-primary/10 border-primary/50 ring-2 ring-primary/30'
                    : 'bg-white/5 border-white/10 hover:border-white/20'
                }`}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {TYPOGRAPHY_OPTIONS.slice(0, 8).map((font, index) => {
            ...
            return (
              ...
              >
                <p className="text-xs sm:text-sm font-medium">{font}</p>
                {isSelected && (
                  <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-primary flex items-center justify-center">
                    <Check className="w-3 h-3 text-primary-foreground" />
                  </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {VISUAL_STYLES.slice(0, 6).map((style, index) => {
            ...
            return (
              ...
              >
                <p className="text-xs sm:text-sm font-medium">{style}</p>
                {isSelected && (
                  <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-primary flex items-center justify-center">
                    <Check className="w-3 h-3 text-primary-foreground" />
                  </div>
