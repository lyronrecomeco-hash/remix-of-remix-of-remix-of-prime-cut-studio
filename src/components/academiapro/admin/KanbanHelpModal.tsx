...
const helpContent = {
  workouts: {
    ...
    columns: [
      { name: 'Rascunho', color: 'bg-muted-foreground', description: 'Treinos em criação ou edição' },
      { name: 'Pronto', color: 'bg-blue-500', description: 'Treinos prontos para serem atribuídos' },
      { name: 'Atribuído', color: 'bg-primary', description: 'Treinos já atribuídos a alunos' },
  ...
  classes: {
    ...
    columns: [
      { name: 'Inativas', color: 'bg-muted-foreground', description: 'Aulas desativadas temporariamente' },
      ...
      { name: 'Lotadas', color: 'bg-primary', description: 'Aulas com capacidade máxima atingida' },
    ],
    tips: [
  ...
  attendance: {
    ...
    columns: [
      { name: 'Aguardando', color: 'bg-muted-foreground', description: 'Alunos que ainda não chegaram' },
  ...
  financial: {
    ...
    columns: [
      ...
      { name: 'Cancelado', color: 'bg-muted-foreground', description: 'Pagamento cancelado' },
    ],
    tips: [
...
export function KanbanHelpModal({ open, onOpenChange, type }: KanbanHelpModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border text-foreground max-w-lg max-h-[85vh] overflow-hidden flex flex-col fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%] z-[100]">
        ...
        <div className="flex-1 overflow-y-auto space-y-6 pr-2 mt-4">
          ...
          <div className="bg-muted/50 border border-border rounded-xl p-4">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-sm text-muted-foreground">para mudar status</span>
            </div>
            <div className="flex gap-2 overflow-x-auto pb-1">
              {content.columns.map((col) => (
                <div 
                  key={col.name}
                  className="flex items-center gap-1.5 px-2 py-1 bg-background rounded-lg flex-shrink-0"
                >
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-sm text-foreground/80">Status disponíveis:</h3>
            {content.columns.map((col) => (
              <div 
                key={col.name}
