...
const WhatsAppAutomation = () => {
  ...
  const renderContent = () => {
    const instancesProps = instances.map(i => ({ id: i.id, name: i.name, status: i.status }));
    
    switch (activeSection) {
      case 'dashboard':
        return <WADashboard instances={instances.map(i => ({ id: i.id, name: i.name, status: i.status, last_heartbeat_at: i.last_heartbeat_at, uptime_seconds: i.uptime_seconds }))} isBackendActive={isBackendActive} />;
      case 'inbox':
        return <WAInbox instances={instancesProps} />;
      case 'instances':
        return <WAInstances instances={instances} isBackendActive={isBackendActive} backendMode={backendMode} backendUrl={backendUrl} localEndpoint={localEndpoint} localPort={localPort} localToken={localToken} masterToken={masterToken} isLocalConnected={isLocalConnected} onRefresh={fetchData} />;
      case 'send':
