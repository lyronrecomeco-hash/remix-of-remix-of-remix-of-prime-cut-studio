// Genesis Professional Features - Organized Exports

// Core Features
export { SecuritySettings } from './SecuritySettings';
export { WorkScheduleSettings } from './WorkScheduleSettings';
export { BrandingSettings } from './BrandingSettings';

// AI Assistant
export { AIAssistant } from './ai/AIAssistant';

// Team Management
export { DepartmentsManager } from './team/DepartmentsManager';
export { RoutingRules } from './team/RoutingRules';
export { InternalChat } from './team/InternalChat';

// Customer Experience
export { NPSSurveys } from './customer/NPSSurveys';
export { ContactAssignments } from './customer/ContactAssignments';
export { ClosureReasons } from './customer/ClosureReasons';
export { QuickRepliesManager } from './customer/QuickRepliesManager';

// Reports
export { SyntheticReport, AnalyticReport } from './reports';
