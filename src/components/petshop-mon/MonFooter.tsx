...
const MonFooter = () => {
  const services = [
    ...
    { name: 'Tosa Especializada', href: '#servicos' },
    { name: 'Hospedagem', href: '#servicos' },
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-10 mb-8 sm:mb-12">
          ...
          <div className="col-span-2 md:col-span-1">
            <div className="flex items-center gap-2 sm:gap-3 mb-4">
              <motion.div 
                className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg shadow-cyan-500/30"
                whileHover={{ scale: 1.05, rotate: 5 }}
              >
                <span className="text-xl sm:text-2xl">🐾</span>
              </motion.div>
              <div>
                <h3 className="text-lg sm:text-xl font-bold text-white">Mon Petit</h3>
                <p className="text-[10px] sm:text-xs text-cyan-400 font-semibold">ARACRUZ</p>
              </div>
            </div>
            <p className="text-white/60 text-xs sm:text-sm mb-4 sm:mb-6 leading-relaxed max-w-xs">
              O banho e tosa mais cheiroso da cidade! Ambiente climatizado e espaço pet. 🐕❤️
            </p>
            <a
            ...
            <ul className="space-y-2 sm:space-y-3">
              {services.map((service) => (
                <li key={service.name}>
                  <a 
                    href={service.href} 
                    className="text-white/60 hover:text-cyan-400 transition-colors text-xs sm:text-sm flex items-center gap-1 group"
                  >
            ...
            <ul className="space-y-2 sm:space-y-3">
              <li>
                <a 
                  ...
                  className="flex items-center gap-2 text-white/60 hover:text-cyan-400 transition-colors text-xs sm:text-sm"
                >
                  <Phone className="w-3.5 h-3.5 sm:w-4 sm:h-4 flex-shrink-0" />
                  <span>(27) 99771-7391</span>
                </a>
              </li>
              <li>
                <a 
        ...
        <div className="pt-6 sm:pt-8 border-t border-white/10">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 sm:gap-4">
            <p className="text-[10px] sm:text-sm text-white/40 text-center sm:text-left">
              © {new Date().getFullYear()} Mon Petit Aracruz. Todos os direitos reservados.
            </p>
