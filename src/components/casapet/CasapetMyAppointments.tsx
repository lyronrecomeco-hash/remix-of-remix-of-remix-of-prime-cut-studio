...
import { X, Calendar, Clock, MapPin, Stethoscope, ShoppingBag, Syringe, Loader2, AlertCircle } from 'lucide-react';
...
interface CasapetMyAppointmentsProps {
  isOpen: boolean;
  onClose: () => void;
}
...
export function getCasapetAppointments(): Appointment[] {
  try {
...
function persistAppointments(appointments: Appointment[]) {
  localStorage.setItem(CASAPET_STORAGE_KEY, JSON.stringify(appointments));
  window.dispatchEvent(new Event('storage'));
}

const CasapetMyAppointments = ({ isOpen, onClose }: CasapetMyAppointmentsProps) => {
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [cancellingId, setCancellingId] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen) {
      setAppointments(getCasapetAppointments());
    }
  }, [isOpen]);
  ...
  const handleCancel = async (apt: Appointment) => {
    ...
    const petshopPhone = '5528999999999';

    const cancelToPetshop = `Olá! Gostaria de cancelar meu agendamento na *Casa Pet Gri*. ❌\n\n*Agendamento:*\n• Serviço: ${apt.serviceName}\n• Pet: ${apt.petName} ${apt.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(apt.date)} às ${apt.time}\n\n*Meus dados:*\n• Nome: ${apt.ownerName}\n• WhatsApp: ${apt.phone}\n\nObrigado(a)!`;

    const cancelToClient = `❌ *Cancelamento confirmado!*\n\nOlá, ${apt.ownerName}! Seu cancelamento na *Casa Pet Gri* foi registrado.\n\n• Serviço: ${apt.serviceName}\n• Pet: ${apt.petName} ${apt.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(apt.date)} às ${apt.time}\n\nSe quiser remarcar, é só fazer um novo agendamento. Tudo o que seu pet precisa nós temos aqui! 🐾`;
    ...
    try {
      ...
      const next: Appointment[] = getCasapetAppointments().map((a) =>
        a.id === apt.id
  ...
  const openGPS = () => {
    const address = encodeURIComponent('Av. Padre José Anchieta, 1030, Guarapari - ES');
        ...
        >
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-4 sm:p-5 text-white relative">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs sm:text-sm font-medium">Casa Pet Gri</span>
            </div>
            ...
            {appointments.length === 0 ? (
              <div className="text-center py-8 sm:py-12">
                <div className="w-14 h-14 sm:w-16 sm:h-16 bg-emerald-100 rounded-full flex items-center justify-center mx-auto mb-3 sm:mb-4">
                  <Calendar className="w-7 h-7 sm:w-8 sm:h-8 text-emerald-500" />
                </div>
                <p className="text-gray-600 text-sm sm:text-base">Você ainda não tem agendamentos</p>
                <p className="text-gray-400 text-xs sm:text-sm mt-1">Agende uma consulta para seu pet!</p>
              </div>
            ) : (
                  ...
                  <div>
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {activeAppointments.map((apt) => {
                        const Icon = serviceIcons[apt.service] || Stethoscope;
                        return (
                          <motion.div
                            key={apt.id}
                          ...
                          >
                            ...
                            <div className="flex gap-2 mt-3">
                              <Button
                                ...
                                size="sm"
                                onClick={openGPS}
                                className="flex-1 h-9 text-xs sm:text-sm border-emerald-300 text-emerald-600 hover:bg-emerald-50"
                              >
          ...
          <div className="p-4 sm:p-5 border-t border-gray-100 bg-gray-50">
            <Button
              onClick={onClose}
              className="w-full h-11 sm:h-12 text-sm sm:text-base font-semibold bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl"
            >
              Fechar
            </Button>
          </div>
...
export default CasapetMyAppointments;
