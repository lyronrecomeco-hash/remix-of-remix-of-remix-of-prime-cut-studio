...
import { X, ChevronRight, ChevronLeft, Check, Calendar, Clock, User, Phone, Sparkles, Loader2, CheckCircle2, Stethoscope, ShoppingBag, Syringe } from 'lucide-react';
...
interface CasapetScheduleProps {
  isOpen: boolean;
  onClose: () => void;
}

const CASAPET_STORAGE_KEY = 'casapet_appointments';

// Helper functions for Casa Pet appointments
export function getCasapetAppointments() {
  try {
    const stored = localStorage.getItem(CASAPET_STORAGE_KEY);
...
export function saveCasapetAppointment(appointment: any) {
  const current = getCasapetAppointments();
  ...
  localStorage.setItem(CASAPET_STORAGE_KEY, JSON.stringify(current));
  window.dispatchEvent(new Event('storage'));
}

const CasapetSchedule = ({ isOpen, onClose }: CasapetScheduleProps) => {
  ...
  const services = [
    { id: 'consulta', name: 'Consulta Veterinária', price: 'A consultar', duration: '30min', icon: Stethoscope, color: 'bg-emerald-500', description: 'Avaliação completa de saúde' },
    { id: 'vacina', name: 'Vacinação', price: 'A consultar', duration: '15min', icon: Syringe, color: 'bg-teal-500', description: 'Vacinas em dia para seu pet' },
    { id: 'checkup', name: 'Check-up Preventivo', price: 'A consultar', duration: '45min', icon: Stethoscope, color: 'bg-green-500', description: 'Avaliação preventiva completa' },
    { id: 'orientacao', name: 'Orientação Nutricional', price: 'Grátis', duration: '20min', icon: ShoppingBag, color: 'bg-emerald-600', description: 'Dicas de alimentação' },
  ];
  ...
  const handleSubmit = async () => {
    ...
    const message = `Olá! 🐾 Gostaria de agendar um atendimento na *Casa Pet Gri*.
...
Aguardo a confirmação! Obrigado(a)! 😊`;
    ...
    try {
      ...
      const confirmationMessage = `✅ *Agendamento confirmado!*\n\nOlá, ${formData.ownerName}! Seu agendamento na *Casa Pet Gri* foi confirmado.\n\n• Serviço: ${selectedService?.name}\n• Pet: ${formData.petName} ${formData.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(formData.date)} às ${formData.time}\n\n📍 Endereço: Av. Padre José Anchieta, 1030 - Guarapari, ES\n\nTudo o que seu pet precisa nós temos aqui! 🐾`;
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto">
        ...
        >
          ...
          <div className="sticky top-0 pt-3 pb-2 bg-gradient-to-r from-emerald-500 to-teal-500 sm:hidden z-10">
          ...
          {isSuccess ? (
            ...
            <>
              {/* Header */}
              <div className="bg-gradient-to-r from-emerald-500 to-teal-500 p-4 sm:p-5 text-white relative">
                <button 
                  onClick={handleClose} 
                ...
                <h2 className="text-lg sm:text-xl font-bold">Agendar Atendimento</h2>
              ...
              <div className="p-4 sm:p-5 border-t border-gray-100 bg-gray-50 flex gap-3">
                ...
                {step < 4 ? (
                  ...
                  <Button
                    ...
                    className="flex-1 h-11 sm:h-12 text-sm sm:text-base font-semibold bg-teal-600 hover:bg-teal-700 text-white rounded-xl"
                  >
                    {isSubmitting ? (
                      <>
...
export default CasapetSchedule;
