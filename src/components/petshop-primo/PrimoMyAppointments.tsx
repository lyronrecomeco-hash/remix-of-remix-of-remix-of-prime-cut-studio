...
import { X, Calendar, Clock, MapPin, Scissors, Stethoscope, Home, Pill, Loader2, AlertCircle } from 'lucide-react';
...
interface PrimoMyAppointmentsProps {
  isOpen: boolean;
  onClose: () => void;
}
...
const serviceNames: Record<string, string> = {
  ...
  farmacia: 'Farmácia Veterinária',
  hotel: 'Hotel/Creche',
};

export function getPrimoAppointments(): Appointment[] {
  try {
...
function persistAppointments(appointments: Appointment[]) {
  localStorage.setItem(PRIMO_STORAGE_KEY, JSON.stringify(appointments));
  window.dispatchEvent(new Event('storage'));
}

const PrimoMyAppointments = ({ isOpen, onClose }: PrimoMyAppointmentsProps) => {
  const [appointments, setAppointments] = useState<Appointment[]>([]);
  const [cancellingId, setCancellingId] = useState<string | null>(null);

  useEffect(() => {
    if (isOpen) {
      setAppointments(getPrimoAppointments());
    }
  }, [isOpen]);
  ...
  const handleCancel = async (apt: Appointment) => {
    ...
    const cancelToPetshop = `Olá! Gostaria de cancelar meu agendamento no *PrimoPet Recife*. ❌\n\n*Agendamento:*\n• Serviço: ${apt.serviceName}\n• Pet: ${apt.petName} ${apt.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(apt.date)} às ${apt.time}\n\n*Meus dados:*\n• Nome: ${apt.ownerName}\n• WhatsApp: ${apt.phone}\n\nObrigado(a)!`;

    const cancelToClient = `❌ *Cancelamento confirmado!*\n\nOlá, ${apt.ownerName}! Seu cancelamento no *PrimoPet Recife* foi registrado.\n\n• Serviço: ${apt.serviceName}\n• Pet: ${apt.petName} ${apt.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(apt.date)} às ${apt.time}\n\nSe quiser remarcar, é só fazer um novo agendamento. Somos todos primos! 🐾`;
    ...
    try {
      ...
      const next: Appointment[] = getPrimoAppointments().map((a) =>
        a.id === apt.id
  ...
  const openGPS = () => {
    const address = encodeURIComponent('Av. Visconde de Albuquerque, 894, Recife - PE, 50610-090');
        ...
        >
          ...
          <div className="bg-gradient-to-r from-emerald-500 to-green-500 p-4 sm:p-5 text-white relative">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs sm:text-sm font-medium">PrimoPet Recife</span>
            </div>
...
export default PrimoMyAppointments;
