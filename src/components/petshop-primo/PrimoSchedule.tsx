...
import { X, ChevronRight, ChevronLeft, Check, Calendar, Clock, User, Phone, Sparkles, Loader2, CheckCircle2, Scissors, Stethoscope, Home, Pill } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { toast } from 'sonner';
import { saveAppointment } from '@/components/petshop/PetshopMyAppointments';
import { openWhatsAppLink, sendPetshopWhatsAppWithRetry } from '@/lib/petshopWhatsApp';

interface PrimoScheduleProps {
  isOpen: boolean;
  onClose: () => void;
}

const PRIMO_STORAGE_KEY = 'primopet_appointments';

// Helper functions for PrimoPet appointments
export function getPrimoAppointments() {
  try {
    const stored = localStorage.getItem(PRIMO_STORAGE_KEY);
...
export function savePrimoAppointment(appointment: any) {
  const current = getPrimoAppointments();
  ...
  localStorage.setItem(PRIMO_STORAGE_KEY, JSON.stringify(current));
  window.dispatchEvent(new Event('storage'));
}

const PrimoSchedule = ({ isOpen, onClose }: PrimoScheduleProps) => {
  ...
  const services = [
    { id: 'banho', name: 'Banho Completo', price: 'R$ 60', duration: '1h', icon: Scissors, color: 'bg-emerald-500', description: 'Banho, secagem e perfume' },
    { id: 'tosa', name: 'Banho + Tosa', price: 'R$ 90', duration: '2h', icon: Scissors, color: 'bg-green-600', description: 'Banho completo + tosa estética' },
    { id: 'vet', name: 'Consulta Veterinária', price: 'R$ 150', duration: '30min', icon: Stethoscope, color: 'bg-teal-500', description: 'Consulta com veterinário' },
    { id: 'farmacia', name: 'Farmácia Veterinária', price: 'Consultar', duration: 'Variável', icon: Pill, color: 'bg-lime-500', description: 'Medicamentos e produtos' },
    { id: 'hotel', name: 'Hotel/Creche', price: 'R$ 80/dia', duration: '24h', icon: Home, color: 'bg-cyan-500', description: 'Hospedagem com cuidados' },
  ];

  const times = ['07:30', '08:00', '09:00', '10:00', '11:00', '14:00', '15:00', '16:00', '17:00', '18:00'];
  ...
  const handleSubmit = async () => {
    ...
    const message = `Olá! 🐾 Gostaria de agendar um serviço para meu pet no *PrimoPet Recife*.
...
Aguardo a confirmação! Obrigado(a)! 😊`;
    ...
    try {
      const petshopPhone = '558130907777';
      ...
      try {
        ...
        console.log('✅ Mensagem enviada via backend (PrimoPet)');
      } catch (e) {
        console.error('Erro ao enviar WhatsApp (PrimoPet):', e);
        openWhatsAppLink(petshopPhone, message);
      }

      const confirmationMessage = `✅ *Agendamento confirmado!*\n\nOlá, ${formData.ownerName}! Seu agendamento no *PrimoPet Recife* foi confirmado.\n\n• Serviço: ${selectedService?.name}\n• Pet: ${formData.petName} ${formData.petType === 'dog' ? '🐕' : '🐱'}\n• Data/Hora: ${formatDate(formData.date)} às ${formData.time}\n\n📍 Endereço: Av. Visconde de Albuquerque, 894, Recife - PE\n\nSomos todos primos! 🐾`;
      ...
      openWhatsAppLink('558130907777', message);
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm overflow-y-auto">
        ...
        >
          ...
          <div className="sticky top-0 pt-3 pb-2 bg-gradient-to-r from-emerald-500 to-green-500 sm:hidden z-10">
          ...
          {isSuccess ? (
            ...
            >
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-3 sm:p-4 mb-4 sm:mb-6 text-left">
              ...
              <Button
                onClick={handleClose}
                className="w-full h-12 text-base font-semibold bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl"
              >
                Fechar
              </Button>
            </motion.div>
            ...
            <>
              {/* Header */}
              <div className="bg-gradient-to-r from-emerald-500 to-green-500 p-4 sm:p-5 text-white relative">
                <button 
                  onClick={handleClose} 
                ...
                <AnimatePresence mode="wait">
                      ...
                      {services.map((service, index) => (
                        ...
                        >
                          ...
                          <div className="text-right flex-shrink-0">
                            <p className="font-bold text-emerald-600 text-sm sm:text-base">{service.price}</p>
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          className="h-11 sm:h-12 rounded-xl border-gray-200 bg-white text-gray-900 placeholder:text-gray-400 focus:border-emerald-500 focus:ring-emerald-500"
                        />
                      </div>
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <Input 
                          ...
                          min={getMinDate()} 
                          className="h-11 sm:h-12 rounded-xl border-gray-200 bg-white text-gray-900 focus:border-emerald-500 focus:ring-emerald-500"
                        />
                      </div>
                  ...
                  {step === 4 && (
                    ...
                    >
                      ...
                      {/* Summary */}
                      <div className="bg-emerald-50 rounded-xl p-3 sm:p-4 mt-3 sm:mt-4">
                        ...
                        <div className="space-y-1.5 sm:space-y-2 text-xs sm:text-sm">
                          ...
                          <div className="flex justify-between pt-2 border-t border-emerald-200">
                            ...
                            <span className="font-bold text-emerald-600">{services.find(s => s.id === formData.service)?.price}</span>
                          </div>
                        </div>
                      </div>
                    </motion.div>
              ...
              <div className="p-4 sm:p-5 border-t border-gray-100 bg-gray-50 flex gap-3">
                ...
                {step < 4 ? (
                  <Button
                    onClick={handleNext}
                    className="flex-1 h-11 sm:h-12 text-sm sm:text-base font-semibold bg-emerald-500 hover:bg-emerald-600 text-white rounded-xl"
                  >
                    Próximo
                  ...
                  <Button
                    ...
                    className="flex-1 h-11 sm:h-12 text-sm sm:text-base font-semibold bg-green-600 hover:bg-green-700 text-white rounded-xl"
                  >
                    {isSubmitting ? (
                      <>
